...
- **An Agent Skill, generated from the docs.** `@cocoar/vue-ui` ships
  ...
  as a reference file with its demos inlined as `vue` code blocks. Install it
  with `npx skills add cocoar-dev/cocoar-ui-vue` (or from the installed
  package: `npx skills add ./node_modules/@cocoar/vue-ui`; with agentskills-cli:
  `agentskills-cli add @cocoar/vue-ui`). Generated by `pnpm skill:sync`,
  verified by CI (`pnpm skill:check`).
