...
- **An Agent Skill, generated from the docs.** `@cocoar/vue-ui` ships
  ...
  as a reference file with its demos inlined as `vue` code blocks.
  `npx @cocoar/vue-ui skill` installs the skill of the installed version
  through the skills CLI (options pass through); `npx skills add
  cocoar-dev/cocoar-ui-vue` and `agentskills-cli add @cocoar/vue-ui` work
  too. Generated by `pnpm skill:sync`, verified by CI (`pnpm skill:check`).
