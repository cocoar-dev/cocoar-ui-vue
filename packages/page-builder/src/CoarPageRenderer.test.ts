...
/**
 * The host locks a node so a customization cannot make it disappear. The
 * publication gate can only see the document — Element Code arrives afterwards,
 * as `pageCodeValues`. If the lock stopped there it would guard nothing.
 */
describe('CoarPageRenderer — required-node locks survive Element Code', () => {
  const schema = {
    id: 'root',
    type: 'page',
    children: [
      { id: 'legal', type: 'note', name: 'legal', props: { text: 'Legally required notice' } },
      { id: 'free', type: 'note', name: 'free', props: { text: 'Ordinary note' } },
    ],
  } as unknown as PageNode;

  const config: PageConfig = {
    requiredNodes: [{ id: 'legal', type: 'note', lockVisibility: true }],
  };

  it('keeps a visibility-locked node rendered when code hides it', () => {
    const wrapper = mount(CoarPageRenderer, {
      props: {
        schema,
        config,
        pageCodeValues: {
          nodes: {
            legal: { style: { hidden: true } },
            free: { style: { hidden: true } },
          },
        } as never,
      },
    });

    expect(wrapper.text()).toContain('Legally required notice');
    // The lock is targeted: an unlocked node still obeys the same code.
    expect(wrapper.text()).not.toContain('Ordinary note');
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('locked required node'));
  });

  it('keeps it rendered when code adds a condition that does not hold', () => {
    const wrapper = mount(CoarPageRenderer, {
      props: {
        schema,
        config,
        pageCodeValues: {
          nodes: { legal: { visibleWhen: { field: 'nothing', equals: true } } },
        } as never,
      },
    });
  ...
  it('lets code style a locked node in every way the lock does not cover', () => {
    const wrapper = mount(CoarPageRenderer, {
      props: {
        schema,
        config,
        pageCodeValues: { nodes: { legal: { style: { padding: '32px' } } } } as never,
      },
    });

    expect(wrapper.text()).toContain('Legally required notice');
    expect(wrapper.html()).toContain('padding: 32px');
  });
});
