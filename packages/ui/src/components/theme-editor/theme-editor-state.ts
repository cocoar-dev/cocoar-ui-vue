...
export interface SemanticEntry { token: string; label: string; defaultPalette: SemanticPalette; defaultStep: number; }
export interface SemanticGroup { label: string; entries: SemanticEntry[]; }
export interface SemanticOverride { palette: SemanticPalette; step: number; }
...
export const FONT_OPTIONS_BODY_SELECT  = computed<CoarSelectOption<string>[]>(() => FONT_OPTIONS_BODY.map(f => ({ value: f, label: f })));
export const FONT_OPTIONS_TITLE_SELECT = computed<CoarSelectOption<string>[]>(() => FONT_OPTIONS_TITLE.map(f => ({ value: f, label: f })));
export const SEMANTIC_PAL_OPTIONS      = computed<CoarSelectOption<string>[]>(() => SEMANTIC_PALETTES.map(p => ({ value: p, label: SEMANTIC_PALETTE_LABELS[p] })));
export const SEMANTIC_STEP_OPTIONS     = computed<CoarSelectOption<number>[]>(() => SEMANTIC_STEPS.map(s => ({ value: s, label: String(s) })));
...
export function setSemantic(entry: SemanticEntry, field: 'palette' | 'step', value: string | number) {
...
export function initThemeEditorState(hideDarkToggle: boolean) {
  ...
  const str  = (t: string, ref_: Ref<string>) => { const v = get(t); if (v) ref_.value = v; };
