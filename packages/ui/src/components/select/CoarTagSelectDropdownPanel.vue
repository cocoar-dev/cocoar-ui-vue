...
/**
 * Panel mounted by the overlay-service when `CoarTagSelect` opens. Similar to the
 * single-select panel but with no `isSelected` / checkmark markup — selected values
 * live as tags in the trigger, not as highlighted rows in the dropdown. The dropdown
 * is only opened when `availableOptions` is non-empty, so an explicit empty state
 * would never render; we keep the class anyway for style parity with the siblings.
 */
