 ...
 *
 * Designed for the v2 timeline view (resources × time) but shipped
 * with v1 so the architecture commitment from the design doc §0.6 is
 * empirically validated and not just promised.
 *
 ...
 * partial re-layouts). Real consumers — the v2 timeline view in
 * particular — typically have known cell sizes (a "minute" is a
 * fixed pixel width; a "resource" is a fixed row height). We ship
 * fixed-size 2D in Phase 0; variable-size 2D extends the API when
 * v2 demands it.
 *
