...
function resolveAgendaSpan(
  ...
  if (allDay) {
    const firstDay = Temporal.PlainDate.from(event.start.slice(0, 10));
    let lastDayInclusive: Temporal.PlainDate;
    if (event.end) {
      const endExcl = Temporal.PlainDate.from(event.end.slice(0, 10));
  ...
  if (event.end) {
    const endDay = eventStartDateInZone(event.end, timezone);
...
function eventStartTimestamp(event: CalendarEvent, timezone: string): number {
  if (isDateOnlyIsoString(event.start)) {
    const d = Temporal.PlainDate.from(event.start);
    return d.toZonedDateTime({ timeZone: timezone }).epochMilliseconds;
  }
  const parsed = parseEventInstant(event.start);
  if (parsed.kind === 'instant') return parsed.instant.epochMilliseconds;
  if (parsed.kind === 'plain') {
    return parsed.plainDateTime.toZonedDateTime(timezone).epochMilliseconds;
  }
  return parsed.date.toZonedDateTime({ timeZone: timezone }).epochMilliseconds;
}
