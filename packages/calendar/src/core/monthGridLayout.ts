...
function resolveDayBounds(
  ...
  if (allDay) {
    const firstDay = parsePlainDateLoose(event.start, timezone);
    let lastDayInclusive: Temporal.PlainDate;
    if (event.end) {
      const endExcl = parsePlainDateLoose(event.end, timezone);
  ...
  // Timed event. Single-day in month-view terms — pin to start day.
...
function parsePlainDateLoose(iso: string, timezone: string): Temporal.PlainDate {
  if (isDateOnlyIsoString(iso)) return Temporal.PlainDate.from(iso);
  return eventStartDateInZone(iso, timezone);
}
