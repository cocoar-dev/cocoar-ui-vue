...
describe('eventStartDateInZone', () => {
  it('PlainDate input is returned as-is regardless of zone', () => {
    const date = Temporal.PlainDate.from('2026-04-13');
    expect(eventStartDateInZone(date, 'Europe/Vienna').toString()).toBe(
      '2026-04-13',
    );
    expect(eventStartDateInZone(date, 'America/Los_Angeles').toString()).toBe(
      '2026-04-13',
    );
  });
  it('ZonedDateTime is re-zoned then truncated to the calendar day', () => {
    // 23:30 UTC = 00:30 next day in Vienna (CET / UTC+1).
    const zdt = Temporal.ZonedDateTime.from('2026-01-13T23:30:00[UTC]');
    expect(eventStartDateInZone(zdt, 'Europe/Vienna').toString()).toBe(
      '2026-01-14',
    );
    expect(eventStartDateInZone(zdt, 'America/Los_Angeles').toString()).toBe(
      '2026-01-13',
    );
  });
});
