...
describe('computeViewWindow — day', () => {
  it('start == cursor, end == cursor + 1 day', () => {
    ...
    const w = computeViewWindow({ view: 'day', cursor, firstDayOfWeek: 1 });
    expect(w).toEqual({ view: 'day', start: '2026-04-15', end: '2026-04-16' });
  });
});
...
describe('computeViewWindow — week (Monday-start)', () => {
  it('Wednesday cursor anchors to the prior Monday, end the next Monday', () => {
    ...
    const w = computeViewWindow({ view: 'week', cursor, firstDayOfWeek: 1 });
    expect(w).toEqual({ view: 'week', start: '2026-04-13', end: '2026-04-20' });
  });
});
...
describe('computeViewWindow — week (Sunday-start)', () => {
  it('Wednesday cursor anchors to the prior Sunday', () => {
    const cursor = Temporal.PlainDate.from('2026-04-15');
    const w = computeViewWindow({ view: 'week', cursor, firstDayOfWeek: 0 });
    expect(w).toEqual({ view: 'week', start: '2026-04-12', end: '2026-04-19' });
  });
});
...
describe('computeViewWindow — month', () => {
  it('full 6×7 grid window (42 days)', () => {
    ...
    const cursor = Temporal.PlainDate.from('2026-04-15');
    const w = computeViewWindow({ view: 'month', cursor, firstDayOfWeek: 1 });
...
describe('computeViewWindow — agenda', () => {
  it('default 30-day window forward', () => {
    const cursor = Temporal.PlainDate.from('2026-04-15');
    const w = computeViewWindow({ view: 'agenda', cursor, firstDayOfWeek: 1 });
    expect(w).toEqual({
      view: 'agenda',
      start: '2026-04-15',
      end: '2026-05-15',
