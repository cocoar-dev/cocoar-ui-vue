...
import { Temporal, dateKey } from './temporal';
...
export function layoutDayEvents<TMeta extends Record<string, unknown> = Record<string, unknown>>(
  ...
  for (const event of events) {
    ...
    if (!isTimedEvent(event)) continue; // all-day handled elsewhere
  ...
  for (const comp of components) {
    ...
    // If `priorityId` lives in this component, rotate the
    // assignment so it lands on the RIGHTMOST lane (laneCount-1).
    // Lanes priorityLane+1..laneCount-1 shift down by one; lanes
    // < priorityLane stay. This is a valid permutation: any two
    // events that didn't share a lane before still don't after.
    let priorityLane = -1;
    if (opts.priorityId !== undefined) {
      for (const bar of result.bars) {
        if (bar.id === opts.priorityId) {
          priorityLane = bar.lane;
          break;
        }
      }
    }
    const lastLane = result.laneCount - 1;
    for (const bar of result.bars) {
      let lane = bar.lane;
      if (priorityLane >= 0 && priorityLane < lastLane) {
        if (lane === priorityLane) lane = lastLane;
        else if (lane > priorityLane) lane = lane - 1;
      }
      laneById.set(bar.id, { lane, laneCount: result.laneCount });
    }
  }
...
// ─── Internal helper for time-grid ───────────────────────────────────

/**
 * Project a `ZonedDateTime` (event start/end) to a minute-of-day for
 * a specific calendar day in the DISPLAY timezone.
 *
 * The event's source-zone is honoured by re-zoning into the display
 * zone first, so cross-zone events render at the correct visual slot.
 *
 * Returns `-1` when the instant is before `day`'s start in the display
 * zone (caller clips to top), or `24*60+1` when after the day ends
 * (caller clips to bottom).
 */
function projectToDayMinute(
  ...
  const zdt = start.withTimeZone(timezone);

  // Audit Session 3 #8 fix — explicit disambiguation. Polyfill default
  // for `PlainDate.toZonedDateTime({ plainTime: undefined })` is
  // 'compatible'; making it explicit guards against future zones with
  // non-midnight DST gaps (Casey, hypothetical Lord_Howe variants)
  // shifting day-start away from 00:00.
  const dayStart = Temporal.PlainDateTime.from({
    year: day.year, month: day.month, day: day.day, hour: 0, minute: 0,
  }).toZonedDateTime(timezone, { disambiguation: 'compatible' });
  const dayEnd = dayStart.add({ days: 1 });

  if (Temporal.ZonedDateTime.compare(zdt, dayStart) < 0) return -1;
  if (Temporal.ZonedDateTime.compare(zdt, dayEnd) >= 0) return 24 * 60 + 1;
