/**
 * `useTimelineView` — convenience composable that pre-locks the
 * active view to `'timeline'`. See `useDayView.ts` for the
 * rationale.
 */
...
>(): {
  ...
  builder.view(ref('timeline'));
  builder.availableViews(['timeline']);
  return { builder, api };
}
